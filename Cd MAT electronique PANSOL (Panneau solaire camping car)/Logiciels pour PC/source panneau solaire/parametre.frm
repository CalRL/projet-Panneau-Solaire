...
Private Sub Form_Load()
    ...
    Port = panneau.MSComm1.CommPort
    configuration.ComPort(Port).Value = True   ' Définit le bouton d'option du port.

    ' Obtient le débit en Baud courant.
    FirstComma = InStr(panneau.MSComm1.Settings, ",")
    Baud$ = Left(panneau.MSComm1.Settings, FirstComma - 1)
    ...
    DBits$ = Mid$(panneau.MSComm1.Settings, SecondComma + 1, 1)
    ...
    SBits$ = Mid$(panneau.MSComm1.Settings, ThirdComma + 1, 1)
...
Private Sub OkButton_Click()
    ...
    OldPort = panneau.MSComm1.CommPort
    If NewPort <> OldPort Then                  ' Si le numéro du port change, ferme
                                                ' l 'ancien port.
        If panneau.MSComm1.PortOpen Then
           panneau.MSComm1.PortOpen = False
           ReOpen = True
        End If

        panneau.MSComm1.CommPort = NewPort          ' Définit le nouveau numéro du port.

        If Err = 0 Then
           If ReOpen Then
              panneau.MSComm1.PortOpen = True
           End If
        End If
        If Err Then
           MsgBox Error$, 48
           panneau.MSComm1.CommPort = OldPort
           Exit Sub
        End If
    End If
    
    panneau.MSComm1.Settings = NewBaud$ + "," + NewParity$ + "," + NewData$ + "," + NewStop$
    If Err Then
       MsgBox Error$, 48
       Exit Sub
    End If
    panneau.MSComm1.Handshaking = NewShake
    If Err Then
       MsgBox Error$, 48
       Exit Sub
    End If
